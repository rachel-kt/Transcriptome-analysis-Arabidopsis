setwd("../Final_Shot_25_04_2018/Stress")
library(DESeq2)
library(edgeR)
library(Biobase)
library(ggplot2)
...
colnames(ht_seq_counts) = c("3A","3B","4A","4B","5A","5B","6A","6B")
head(ht_seq_counts)
tail(ht_seq_counts)
...
dds$condition <- factor(dds$condition, levels = c("WTplusK", "WTminusK",
                                                  "MutantplusK", "MutantminusK"))
...
colnames(cluster_matrix_merged) = c("WTplusK", "WTminusK","MutantplusK", "MutantminusK")
head(cluster_matrix_merged)
write.csv(cluster_matrix_merged, "Stress_25-04-2018.csv")
...
colnames(heatmap_data) = c("Col-0 +K","Col-0 -K","cipk9 +K","cipk9 -K", "Cluster")
Heatmap(heatmap_data[1:4], col = f3, cluster_rows = T, show_row_dend = F,
        cluster_columns = F, show_row_names = F, name = "z-score", split = heatmap_data$Cluster, column_order = c(1,3,2,4))
...
colnames(up_genes) = c("WTmk_cipkpk","WTmk_cipkmk", "cipkpk_cipkmk", "WTmk_WTpk", "cipkpk_WTpk","cipkmk_WTpk")
colnames(down_genes) = c("WTmk_cipkpk","WTmk_cipkmk", "cipkpk_cipkmk", "WTmk_WTpk", "cipkpk_WTpk","cipkmk_WTpk")
######## cipk9

write.csv(up_genes, file = "up_regulated_genes_4_fc.csv")
write.csv(down_genes, file = "down_regulated_genes_4_fc.csv")

######## PCA of filtered genes

pca_data = count_mat[match(rownames(cluster_genes),rownames(count_mat)),]
dds_pca <- DESeqDataSetFromMatrix(countData = pca_data,
                              colData = col_info, design = ~ condition)
dds_pca$condition <- factor(dds$condition, levels = c("WTplusK", "WTminusK",
                                                  "MutantplusK", "MutantminusK"))
...
X_cor_plot = res_table_final[match(rownames(cluster_genes), rownames(res_table_final)),11]
Y_cor_plot = res_table_final[match(rownames(cluster_genes), rownames(res_table_final)),13]
cor_plot = data.frame(X_cor_plot, Y_cor_plot)
colnames(cor_plot) = c("Log2(Col-0 -K/Col-0 +K)", "Log2(CIPK -K/Col-0 +K)")
...
ggplot(cor_plot, aes(X_cor_plot, Y_cor_plot, color = pc)) +
  ...
  labs(x = "Log2(Col-0 -K/Col-0 +K)", y = "Log2(CIPK9 -K/Col-0 +K)") +
  geom_smooth(method="lm", show.legend = F, se = F, color = "blue") +
  geom_abline(slope = 1, show.legend = F, color = "black")
cor.test(cor_plot$`Log2(Col-0 -K/Col-0 +K)`, cor_plot$`Log2(CIPK -K/Col-0 +K)`,
